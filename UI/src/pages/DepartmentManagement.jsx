          ...
          {loading && departments.length === 0 ? (
            ...
            <div className="p-16 text-center text-gray-500 max-w-md mx-auto space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="font-extrabold text-sm text-gray-900 uppercase tracking-wide">No Departments Found</h3>
                <p className="text-xs text-gray-400">There are no departments matching your criteria. Add one to get started.</p>
              </div>
              {search && (
                <button
                  onClick={() => setSearch('')}
                    ...
                    <tr key={department.departmentId} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-2.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-extrabold text-gray-900 tracking-tight block">{department.name}</span>
                          </div>
                        </div>
                      </td>
                        ...
                        {department.courses && department.courses.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {department.courses.map((course) => (
                              <span 
                                key={course.id}
                                className="px-2 py-1 bg-gray-100 border border-gray-200 text-gray-650 rounded-md font-bold text-[9px] uppercase tracking-wider"
                              >
                                {course.name}
                              </span>
                            ))}
